...
class TestPost:
    ...
    def test_post_model(self):
        ...
        assert type(text_field) == fields.CharField, (
            'Свойство `text` модели `Post` должно быть текстовым `CharField`'
        )
...
class TestCustomErrorPages:
    ...
    def test_custom_404(self, client):
        ...
        try:
            from kltop.urls import handler404 as handler404_student
        except ImportError:
            assert False, (
    ...
    def test_custom_403(self):
        ...
        try:
            from kltop.urls import handler403
        except ImportError:
            assert False, (
                f'Убедитесь, что для страниц, возвращающих код {code}, '
                'настроен кастомный шаблон'
