...
class TestCreateView:
    ...
    def test_create_view_post(self, mock_media, user_client, user, group):
        ...
        response = user_client.post(url, data={'title': title, 'text': text, 'group': group.id, 'image': image})
        ...
        title = 'Очень важная статья 2!'
        text = 'Проверка нового поста 2!'
        image = self.get_image_file('image2.png')
        response = user_client.post(url, data={'title': title, 'text': text, 'image': image})
        print(response.status_code)
        assert response.status_code in (301, 302), (
            'Проверьте, что со страницы `/create/` после создания второго поста, '
            f'перенаправляете на страницу профиля автора `/profile/{user.username}`'
        )
